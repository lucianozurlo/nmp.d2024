    ...
    function initOnFirstLoad () {
      imagesLoaded ('body', function () {
        ...
        setTimeout (function () {
          ...
          gsap.set ($ ('.showcase-carousel .nmp-slider .slide-inner'), {
            opacity: 0,
          });
          gsap.to (
            $ ('.showcase-carousel .nmp-slider .nmp-slide .slide-inner'),
            {duration: 2, opacity: 1, delay: 0.75, ease: Power4.easeOut}
          );

          gsap.set (
            $ (
              '.showcase-carousel .nmp-slider .nmp-slide-prev-two .slide-inner'
            ),
            {yPercent: 260}
          );
          gsap.to (
          ...
          gsap.set (
            $ ('.showcase-carousel .nmp-slider .nmp-slide-prev .slide-inner'),
            {yPercent: -210}
          );
          gsap.to (
            $ ('.showcase-carousel .nmp-slider .nmp-slide-prev .slide-inner'),
          ...
          gsap.set (
            $ ('.showcase-carousel .nmp-slider .nmp-slide-active .slide-inner'),
            {yPercent: 160}
          );
          gsap.to (
            $ ('.showcase-carousel .nmp-slider .nmp-slide-active .slide-inner'),
          ...
          gsap.set (
            $ ('.showcase-carousel .nmp-slider .nmp-slide-next .slide-inner'),
            {yPercent: -210}
          );
          gsap.to (
            $ ('.showcase-carousel .nmp-slider .nmp-slide-next .slide-inner'),
          ...
          gsap.set (
            $ (
              '.showcase-carousel .nmp-slider .nmp-slide-next-two .slide-inner'
            ),
            {yPercent: 260}
          );
          gsap.to (
          ...
          gsap.set ($ ('.showcase-slider .nmp-slider .slide-inner'), {
            opacity: 0,
          });
          gsap.to ($ ('.showcase-slider .nmp-slider .nmp-slide .slide-inner'), {
            duration: 1.2,
            opacity: 1,
            scale: 1,
            delay: 0.7,
          ...
          gsap.set ($ ('.showcase-gallery .nmp-slider .slide-inner'), {
            opacity: 0,
          });
          gsap.to (
            $ ('.showcase-gallery .nmp-slider .nmp-slide .slide-inner'),
          ...
          gsap.set (
            $ (
              '#filters-wrapper, .nmp-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text'
            ),
            {opacity: 0}
          );
          gsap.to (
...
---------------------------------------------------*/
  ...
  function PageLoadActions () {
    ...
    // Llama a la función después de un cambio AJAX
  ...
  function ShowcaseCarousel () {
    if ($ ('.showcase-carousel').length > 0) {
          ...
          init: function () {
            if ($ ('body').hasClass ('show-loader')) {
              imagesLoaded ('body', function () {
                ...
                var initialHeights = Array.from (bullets).map (function (
                  bullet
                ) {
                  return bullet.clientHeight + 'px';
                });
      ...
      const showGrid = () => {
        ...
        gsap.to ($ ('footer .link-text, .nmp-pagination, .progress-info'), {
          duration: 0.3,
          opacity: 0,
          y: 30,
          stagger: -0.05,
        ...
        gsap.set (currentImage.closest ('.nmp-slide'), {zIndex: 0});
      ...
      const hideGrid = () => {
        gsap.to (
          ...
          {duration: 0.5, opacity: 0, delay: 0.1, ease: Power2.easeOut}
        );
        animateTitle.to (
          '.showcase-carousel .carousel-zoom-wrapper .slide-caption span',
        ...
        Flip.from (flipstate, {
          ...
          onComplete: function () {
            ...
            const triggeredItem = document.querySelector (
              '.nmp-slide.triggered-item'
            );
            if (triggeredItem != null) {
              triggeredItem.classList.remove ('triggered-item');
            }

            const clapatSlides = document.querySelectorAll ('.nmp-slide');
      ...
      const getDOMElements = () => {
        ...
        const items = [];
        [...document.querySelectorAll ('.nmp-slide')].forEach (item => {
          items.push (new Item (item));
        });

        // Cloned items
        const itemsCloned = [];
        [...document.querySelectorAll ('.nmp-slide-clone')].forEach (item => {
        ...
        return {
          ...
          currentItem: new Item (
            document.querySelector ('.nmp-slide.triggered-item')
          ),
      ...
      if (!isMobile ()) {
        ...
        $ ('.nmp-slider').on ('mouseup touchend', function () {
          gsap.to ('#ball', {
            duration: 1,
            borderWidth: '4px',
            scale: 0.5,
        ...
        $ ('.showcase-carousel.preview-mode-enabled .nmp-slide .slide-inner')
        ...
        $ (
          '.showcase-carousel .nmp-slide .slide-inner'
        ).on ('mouseenter', function () {
  ...
  function ShowcaseSlider () {
    if ($ ('.showcase-slider').length > 0) {
        ...
        on: {
          init: function () {
            if ($ ('body').hasClass ('show-loader')) {
              imagesLoaded ('body', function () {
                gsap.to (
                  $ ('.showcase-slider .nmp-slider .nmp-slide .slide-inner'),
                  {
                    duration: 1.2,
                    opacity: 1,
                    scale: 1,
              ...
              let firstSlide = document.querySelector ('.nmp-slide');
      ...
      arrTitles = gsap.utils.toArray (
        '.nmp-slider-wrapper .nmp-sync-slider .nmp-sync-slide'
      );
      ...
      $ ('.trigger-item').on ('click', function () {
        ...
        setTimeout (function () {
          ...
          var clonedListItem = $ (
            '.nmp-sync-slider-viewport .nmp-sync-slide:eq(' + clickedIndex + ')'
          );
  ...
  function ShowcasePortfolio () {
    if ($ ('.showcase-portfolio').length > 0) {
      ...
      function listView () {
        ...
        var stateThumb = Flip.getState ('.nmp-item .slide-inner');

        if (!grid.classList.contains ('list-grid')) {
          if (isMobile ()) {
            gsap.to ($ ('.nmp-item .slide-caption'), {
              duration: 0.5,
              opacity: 0,
              y: 0,
              stagger: 0.05,
          ...
          gsap.to ($ ('.nmp-item .section-image'), {
            duration: 0.5,
            opacity: 0,
            scale: 0.5,
            stagger: 0.05,
            ...
            onComplete: function () {
              ...
              var endHeight = gsap.getProperty (
                '.showcase-portfolio',
                'height'
              );
              ...
              var flip = Flip.from (stateThumb, {
                ...
                onComplete: function () {
                  ...
                  gsap.set ($ ('.nmp-item .section-image'), {width: '100px'});
                },
              });
          ...
          var listMasks = document.querySelectorAll (
            '.nmp-item .slide-inner .list-mask'
          );
      ...
      function shuffleGrid () {
        var grid = document.querySelector ('.showcase-portfolio');
        var startHeight = gsap.getProperty ('.showcase-portfolio', 'height');
        var state = Flip.getState ('.nmp-item');
      ...
      function filter () {
        var projects = document.querySelectorAll ('.nmp-item');
        var startHeight = gsap.getProperty ('.showcase-portfolio', 'height');
        var state = Flip.getState ('.nmp-item');
  ...
  function ShowcaseLists () {
    if ($ ('.showcase-lists').length > 0) {
      ...
      const slidesRoot = document.querySelector (
        '.nmp-sync-slider .nmp-sync-slider-wrapper .nmp-sync-slider-viewport'
      );
      const slidesList = slidesRoot.querySelectorAll (
        '.nmp-sync-slider .nmp-sync-slide'
      );
      ...
      const clapatSyncSlider = document.querySelector (
        '.nmp-slider-wrapper.showcase-lists .nmp-sync-slider-viewport'
      );
      const syncSliderClone = clapatSyncSlider.cloneNode (true);
      document
        .querySelector (
          '.nmp-slider-wrapper.showcase-lists .nmp-sync-slider-wrapper'
        )
      ...
      const syncSliderCloneTranslate = document.querySelectorAll (
        '.nmp-slider-wrapper.showcase-lists .nmp-sync-slider-wrapper > .nmp-sync-slider-viewport'
      );
      const titleWrapper = document.querySelector ('.nmp-sync-slider-wrapper');
      ...
      if (!isMobile ()) {
        ...
        dragWrapper.on ('mousedown', function (evt) {
          dragWrapper.on ('mouseup mousemove', function handler (evt) {
            if (evt.type === 'mouseup') {
              ...
              $ (
                '.showcase-lists .nmp-sync-slide .slide-inner'
              ).on ('click', function () {
                if (!$ ('body').hasClass ('scale-drag-y')) {
                  gsap.to ($ ('.showcase-lists .slide-title span'), {
                    duration: 0.4,
        ...
        $ ('.nmp-sync-slide')
          .on ('mouseenter', function () {
            $ ('.nmp-sync-slide').addClass ('disable');
          ...
          .on ('mouseleave', function () {
            $ ('.nmp-sync-slide').removeClass ('disable');
            $ (this).find ('video').each (function () {
              $ (this).get (0).pause ();
            });
          });
      ...
      Array.from (document.querySelectorAll ('.nmp-sync-slide')).forEach (
...
---------------------------------------------------*/
  ...
  function ShowcaseGallery () {
    if ($ ('.showcase-gallery').length > 0) {
      ...
      gsap.set ($ ('.showcase-gallery .nmp-slider .slide-inner'), {opacity: 0});
      ...
      slider = new ClapatSlider ('.showcase-gallery', {
        ...
        on: {
          ...
          slideLeaveViewport: function (slide) {
            gsap.set (
              $ ('.nmp-slider div:not(.nmp-slide-visible) .slide-events'),
              {y: ''}
            );
          },
        },
      ...
      const showGrid = () => {
        ...
        gsap.to (
          $ (
            'footer .link-text, .nmp-pagination, .progress-info, #filters-wrapper'
          ),
          {
            duration: 0.3,
            opacity: 0,
        ...
        gsap.to ('.nmp-slider .nmp-slide .trigger-item', {
          duration: 1,
          opacity: 0,
          scale: 0.7,
          ease: Power2.easeOut,
      ...
      const initEvents = () => {
        ...
        function moveThumbGrid (direction = 'next') {
          if (document.body.classList.contains ('grid-open')) {
            ...
            // Get all the slides
            let slides = document.querySelectorAll ('.nmp-slide');
            ...
            $ ('.nmp-slide').removeClass ('triggered-item');
            let indexSlide = Number (
              nextImage.getAttribute ('data-slide-index')
            );
      ...
      if (!isMobile ()) {
        if ($ ('.showcase-gallery').hasClass ('tilt-gallery')) {
          ...
          function callTiltSlider (e) {
            moveItSlider (e, '.nmp-slider-viewport', 30);
          }
        ...
        $ ('.showcase-gallery.preview-mode-enabled .nmp-slide .slide-inner')
          .on ('mouseenter', function () {
            if (!$ ('body').hasClass ('scale-drag-x')) {
              $ ('#ball p').remove ();
              var $this = $ (this);
      ...
      if ($ ('#filters-wrapper').length > 0) {
        ...
        $ (
          '#footer-container'
        ).on ('click', '.toggle-filters, #close-filters', function () {
